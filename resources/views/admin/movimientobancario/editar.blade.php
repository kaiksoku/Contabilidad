...
@section('breadcrumbs')
{{ Breadcrumbs::render('movimientobancario.editar', $data) }}
@endsection
            ...
            <div class="col-lg-12">
                ...
                <div class="card card-outline card-success">
                    <div class="card-header">
                        <h3 class="card-title">Editar Movimiento <small>{{$data->movb_descripcion}}</small></h3>
                        <div class="card-tools">
                            <a href="{{route('movimientobancario')}}" class="btn btn-block btn-info btn-sm">
                                Volver a Listado<i class="fas fa-arrow-circle-left pl-1"></i></a>
                        </div>
                    </div>
                    <form action="{{route('movimientobancario.actualizar',['id'=>$data->movb_id])}}" id="form-general" class="form-horizontal" method="POST"
                        autocomplete="off">
                        <div class="card-body">
                            @method('put')
                            @csrf
