...
@section('breadcrumbs')
{{ Breadcrumbs::render('tipocombustible.editar', $data) }}
@endsection
            ...
            <div class="col-lg-12">
                ...
                <div class="card card-outline card-success">
                    <div class="card-header">
                        <h3 class="card-title">Editar Tipo Combustible <small>{{$data->tco_nombre}}</small></h3>
                        <div class="card-tools">
                            <a href="{{route('tipocombustible')}}" class="btn btn-block btn-info btn-sm">
                                Volver a Listado<i class="fas fa-arrow-circle-left pl-1"></i></a>
                        </div>
                    </div>
                    <form action="{{route('tipocombustible.actualizar',['id'=>$data->tco_id])}}" id="form-general" class="form-horizontal" method="POST"
                        autocomplete="off">
                        <div class="card-body">
                            @method('put')
                            @csrf
