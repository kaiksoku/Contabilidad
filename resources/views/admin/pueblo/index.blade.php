...
@section('breadcrumbs')
{{ Breadcrumbs::render('pueblo') }}
@endsection

@section('advertencia')
No cree o actualice un país sin antes haber consultado los manuales del informe del empleador del Ministerio de Trabajo
y Previsión Social.
...
@section('contenido')
<input type="hidden" id="routepath" value="{{url('admin/pueblo')}}">
            ...
            <div class="col-lg-12">
                ...
                <div class="card card-outline card-info">
                    <div class="card-header">
                        <h3 class="card-title">Pueblo <small></small></h3>
                        <div class="card-tools">
                            @can('crear admin/pueblo')
                            <a href="{{route('pueblo.crear')}}" class="btn btn-block btn-success btn-sm">
                                Nuevo registro<i class="fa fa-fw fa-plus-circle pl-1"></i></a>
                            @else
                            <a href="{{route('pueblo.crear')}}" class="btn btn-block btn-success btn-sm disabled">
                                Nuevo registro<i class="fa fa-fw fa-plus-circle pl-1"></i></a>
                            @endcan
                        </div>
                    </div>
                    <div class="card-body">
                        <table class="table table-striped table-hover" id="tabla-data">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$data->pue_id}}</td>
                                    <td>{{$data->pue_descripcion}}</td>
                                    <td>
                                        @can('actualizar admin/pueblo')
                                        <a href="{{route('pueblo.editar',['id'=> $data->pue_id])}}"
                                        ...
                                        <a href="{{route('pueblo.editar',['id'=> $data->pue_id])}}"
                                            ...
                                            <i class="far fa-edit"></i></a>
                                        @endcan
                                        @can('elininar admin/puelo')
                                        <a href="{{route('pueblo.eliminar',['id'=> $data->pue_id])}}"
