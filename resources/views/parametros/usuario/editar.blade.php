...
@section('breadcrumbs')
{{ Breadcrumbs::render('usuario.editar', $data) }}
@endsection
            ...
            <div class="col-lg-12">
                ...
                <div class="card card-outline card-success">
                    <div class="card-header">
                        <h3 class="card-title">Editar Usuario {{$data->usu_nombre}} <small>{{$data->usu_nombre}}</small></h3>
                        <div class="card-tools">
                            <a href="{{route('usuario')}}" class="btn btn-block btn-info btn-sm">
                                Volver a Listado<i class="fas fa-arrow-circle-left pl-1"></i></a>
                        </div>
                    </div>
                    <form action="{{route('usuario.actualizar',['id'=>$data->id])}}" id="form-general" class="form-horizontal" method="POST"
                        autocomplete="off">
                        <div class="card-body">
                            @method('put')
                            @csrf
                            @include('parametros.usuario.forme')
                        </div>
                        <!-- /.card-body -->
