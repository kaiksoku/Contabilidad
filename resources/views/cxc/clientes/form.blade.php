...
<input type="hidden" name="cli_persona" value="{{old('cli_persona',$persona->per_id??'')}}">

<div class="row">
    <div class="col-lg-1"></div>
    <fieldset class="border p-2 col-sm-12 col-lg-10">
        <legend class="w-auto">Datos de Cliente</legend>
        <div class="form-group row">
            <label for="cli_tipoCliente"
                class="col-sm-12 col-lg-2 control-label text-sm-left text-lg-right requerido">Tipo de Cliente</label>
            <div class="col-sm-12 col-lg-4">
                <select name="cli_tipoCliente" id="cli_tipoCliente" class="form-control select2"
                    placeholder="Tipo de Cliente">
                    @foreach ($tipo->getTiposClientes() as $item)
                    <option value="{{$item->tpp_id}}"
                        {{old('cli_tipoCliente',$data->cli_tipoCliente ?? '')==$item->tpp_id ? 'selected':''}}>
            ...
            <label for="cli_credito" class="col-sm-12 col-lg-2 control-label text-sm-left text-lg-right">Días
                de Crédito</label>
            <div class="col-lg-3">
                <input type="number" step="any" min=0 data-decimals=0 name="cli_credito" class="form-control"
                    id="cli_credito" placeholder="0" value="{{old('cli_credito', $data->cli_credito ?? '0')}}">
            </div>
        </div>
    </fieldset>
</div>
...
<div class="row">
    ...
    <fieldset class="border p-2 col-sm-12 col-lg-10">
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-4">
                <input type="text" name="per_cui" class="form-control editable" id="per_cui" placeholder="CUI"
                    value="{{old('per_cui', $persona->per_cui ?? '')}}" onkeypress='return validaNumericos(event,"P");'
                    {{!empty($persona->per_nit)?"disabled":""}}>
            </div>
        </div>
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" class="form-control editable" id="per_nombre" name="per_nombre" placeholder="Nombre"
                    value="{{old('per_nombre',$persona->per_nombre??'')}}" required
                    {{!empty($persona->per_nit)?"disabled":""}}>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-9">
                <input type="text" class="form-control editable" id="per_direccion" name="per_direccion"
                    placeholder="Dirección" value="{{old('per_direccion',$persona->per_direccion??'')}}" required
                    {{!empty($persona->per_nit)?"disabled":""}}>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="col-sm-12 col-lg-6">
                <input type="text" class="form-control editable" id="per_contacto" name="per_contacto"
                    placeholder="Contacto" value="{{old('per_contacto',$persona->per_contacto??'')}}"
                    {{!empty($persona->per_nit)?"disabled":""}}>
            </div>
        </div>
        ...
        <div class="form-group row">
            ...
            <div class="input-group col-sm-12 col-lg-3">
                ...
                <input type="per_email" name="per_email" id="per_email" class="form-control editable"
                    placeholder="Correo Electrónico" value="{{old('per_email',$persona->per_email ?? '')}}"
                    {{!empty($persona->per_nit)?"disabled":""}}>
            </div>
        </div>
