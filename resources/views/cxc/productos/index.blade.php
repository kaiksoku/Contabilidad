            ...
            <div class="col-lg-12">
                ...
                <div class="card card-outline card-info">
                    <div class="card-header">
                        <h3 class="card-title">Productos <small></small></h3>
                        <div class="card-tools">
                            @can('crear cxc/productos')
                            <a href="{{route('productos.crear')}}" class="btn btn-block btn-success btn-sm">
                                Nuevo registro<i class="fa fa-fw fa-plus-circle pl-1"></i></a>
                            @else
                            <a href="{{route('productos.crear')}}" class="btn btn-block btn-success btn-sm disabled">
                                Nuevo registro<i class="fa fa-fw fa-plus-circle pl-1"></i></a>
                            @endcan
                    ...
                    <div class="card-body">
                        <table class="table table-striped table-hover" id="tabla-data">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    <td>{{$data->prod_desc_lg}}</td>
                                    <td>{{$data->prod_desc_ct}}</td>
                                    <td>{{$data->prod_codigo ." - ".$data->Productos->cta_descripcion}}</td>
                                    <td>{{$data->Empresa->emp_siglas}}</td>
                                    <td>{{$data->Terminal->ter_abreviatura}}</td>
                                    <td>
                                       <!--  @can('actualizar cxc/productos')
                                        <a href="{{route('productos.editar',['id'=> $data->prod_id])}}"
                                        ...
                                        <a href="{{route('productos.editar',['id'=> $data->prod_id])}}"
                                            ...
                                            <i class="far fa-edit"></i></a>
                                        @endcan-->
                                        @can('eliminar cxc/productos')
                                        <a href="{{route('productos.eliminar',['id'=> $data->prod_id])}}"
